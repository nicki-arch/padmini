"""Padmini — detecção determinística de fatos notáveis (versão completa para teste)."""
...
COMBUSTAO_LIMIAR = {"Chandra": 12, "Mangala": 17, "Budha": 14, "Guru": 11, "Shukra": 10, "Shani": 15}
...
def _casa_a_partir_de(signo_referencia: str, signo_alvo: str) -> int:
    return ((SIGNOS.index(signo_alvo) - SIGNOS.index(signo_referencia)) % 12) + 1
...
def detectar_dignidades(grahas: dict) -> list[dict]:
    ...
    for nome, regra in DIGNIDADES.items():
        ...
        signo_debil, _ = regra["debilitacao"]
...
def detectar_mangal_dosha(grahas: dict, lagna_signo: str):
...
def detectar_gaja_kesari(grahas: dict):
...
def detectar_neecha_bhanga(grahas: dict, lagna_signo: str) -> list[dict]:
    ...
    for planeta, regra in DIGNIDADES.items():
        ...
        casa_lagna = _casa_a_partir_de(lagna_signo, grahas[dispositor]["signo"])
        casa_lua = _casa_a_partir_de(grahas["Chandra"]["signo"], grahas[dispositor]["signo"])
        if casa_lagna in KENDRAS or casa_lua in KENDRAS:
